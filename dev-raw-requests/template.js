...
$(function () {
    ...
    Module.report.OutpatientVisitRecords = function (data, json) {
        
        var contentList = json.datas.contentList
        var _content0 = '';
        var _medNum = 0;
        var _content1 = '';
        var l = data.length;
        var l0 = contentList.length;

        for (var i = 0; i < l; i++) {
            var drugFlag = Module.getValue(data[i].drugFlag);
            if (Module.getValue(data[i].itemName).includes("诊查费")) {
                continue
            }
        ...
        if (l) {
            _content1 = '<table class="tableInfo tableNowrap">' +
                '<thead>' +
                '<tr>' +
                '<th>名称</th>' +
                '<th>规格</th>' +
        ...
        for (var j = 0; j < l0; j++) {
            var nodeName = contentList[j].nodeName 
            if (!Const.mjzbl_kvs.hasOwnProperty(nodeName)) {
                continue
            }
            var text = contentList[j]["text"]
            if (nodeName == "MedicalHistory"){
                var replaceRegExp = /[；;]/g;
                var replaceWith = "<br>";
                text = text.replace(replaceRegExp, replaceWith);
            }
            
            _content0 += '<div class="report_description1">' +
                '<span class="content_title1">【' + Const.mjzbl_kvs[nodeName] +  '】</span>' +
                '<span class="content_text">' + text + '</span>' +
                '</div>'
        }

        var _html = '<div class="tableWrapper onlyBottomBorder">' +
            '<div class="content_block">' + _content0 + '</div>' +
            '<div class="content_block">' + _content1 + '</div>' +
            '</div>';
    ...
    Module.report.everyday1 = function (data, plt, documentUniqueId) {
        ...
        $.post(navRoot + "/api/inpatient/showinfo/chufang", postData, function (data) {
            ...
            if (data["code"] == 200) {
                ...
                if (plt == "OutpatientVisitRecords") {
                    _html = Module.report[plt]("", jsonForMod);
                }
            }
        }, "json");
...
// 通用脚注
    ...
    Module.normalTable = function (data, withEcharts, empi, payLoadType, resultName, paramName) {
        ...
        for (var k = 0; k < trL; k++) {
            ...
            //// echarts小图标移动
            //var _checkinp = '<input class="check_goEchart" type="checkbox" for-echarts="' + _dataTbody[k][0].text + '" />';
            //_dataTbody[k][0].text = _checkinp + _dataTbody[k][0].text;
